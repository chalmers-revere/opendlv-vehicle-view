...
app.post('/convertrec2csvpng', (req, res) => {
    var process_cluonrec2csv = execSync('rm -f ' + req.body.recordingFile + '.csv.zip && if [ -f external.odvd ]; then cluon-rec2csv --rec=' + req.body.recordingFileToConvert + ' --odvd=external.odvd; else mkdir -p recordings/tmp && cp opendlv-standard-message-set-v0.9.6.odvd recordings/tmp && docker run --rm --volumes-from=opendlv-vehicle-view -w /opt/vehicle-view/recordings/tmp t --rec=../' + req.body.recordingFile + ' --odvd=opendlv-standard-message-set-v0.9.6.odvd && cd recordings/tmp && zip -r9 ../../' + req.body.recordingFile + '.csv.zip *.csv opendlv.proxy.ImageReading* && cd .. && rm -fr tmp && cd ..; fi');
console.log(process_cluonrec2csv.toString());
//    var process_cluonrec2csv = execSync('rm -f ' + req.body.recordingFile + '.csv.zip && if [ -f external.odvd ]; then cluon-rec2csv --rec=' + req.body.recordingFileToConvert + ' --odvd=external.odvd; else ls -l && docker run --rm -v $PWD:/opt/data -w /opt/data t --rec=' + req.body.recordingFileToConvert + ' --odvd=opendlv-standard-message-set-v0.9.6.odvd; fi && zip -r9 ./' + req.body.recordingFile + '.csv.zip *.csv opendlv.proxy.ImageReading*');
// TODO: Test in Docker.
